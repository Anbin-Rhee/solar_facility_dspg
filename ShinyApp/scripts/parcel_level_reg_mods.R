...
final_va_mod_parcels <- readRDS("data/final_parcel_data.rds")
...
parcel_model_full <- feols(log_per_acre ~ log_sf_dist * is_post_period + log_grid_dist * is_post_period + 
                         Acres + composite_flatness_score + dist_road_miles + 
                         dist_water_miles + dist_urban_miles | year + LOCALITY + land_cover_code, 
                       data = final_va_mod_parcels)

# View the results
summary(parcel_model_full, cluster = ~LOCALITY)
